...
def login_view(request, role_code=None):
    ...
    if request.method == 'POST':
        ...
        # 1. Attempt dynamic authentication
        username_field = getattr(User, 'USERNAME_FIELD', 'username')
        user = authenticate(request, **{username_field: username_or_email, 'password': password})

        # 2. Fallback check if user entered email or username alias
        if user is None:
            if '@' in username_or_email:
                ...
                # In custom user model without username, check if username matches test user mapping
                matched_user = User.objects.filter(email__istartswith=f"{username_or_email}@").first()

            if matched_user:
                target_ident = getattr(matched_user, username_field)
                user = authenticate(request, **{username_field: target_ident, 'password': password})
    ...
    default_username = getattr(default_user, 'username', '') if default_user else ''
    if not default_username and default_user:
        default_username = default_user.email.split('@')[0] if '@' in default_user.email else selected_role.code
    ...
    context = {
        ...
        'next_url': next_url,
        'default_username': default_username or selected_role.code,
        'default_email': default_user.email if default_user else f'{selected_role.code}@dineflow.in',
